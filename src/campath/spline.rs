//! Camera-path interpolation, ported 1:1 from the web previewer's `campath.ts` so a
//! path built here renders identically under `mirv_campath`. See that file for the full
//! derivation and HLAE/qspline license notes. The interpolation design mirrors
//! advancedfx shared/AfxMath and McEnnan's CC0 qspline:
...
//!
//! All values are parameterized by keyframe TICK (converted to seconds only on export).
//! Positions/quaternions are stored in Source/Hammer Z-up coords; the renderer converts
//! to/from bevy Y-up at the edges.
